...
async function getPromptId(
  ...
  const data:any=[], error={message:''};/* = await dbClient
    .from("prompt")
    .select("id")
  ...
  if (data !== null && data.length > 0) {
    ...
    if (name) {
      ...
      const { data: highestSuffixData } = {data: {name:''}}/*await dbClient
        .from("prompt")
        .select("name")
    ...
    const { data, error } =  {data:{id:'abc123'},error:''}/*await dbClient
      .from("prompt")
      .insert([
        {
...
async function getHeliconeApiKeyRow(
  ...
  const { data:any, error } = {data:'', error:''}/*await dbClient
...
> {
  try {
    ...
    const formattedPromptResult =
      prompt !== undefined
        ? await getPromptId(prompt, promptName, auth)
        : null;
    if (
    ...
    const { data: heliconeApiKeyRow, error: userIdError } =
      await getHeliconeApiKeyRow(heliconeApiKey);
    if (userIdError !== null) {
    ...
    const { data, error } = {data:'', error:{message:''}}/*await dbClient
      .from("request")
      .insert([
        {
          id: requestId,
    ...
    (async () => {
      ...
      if (requestResult !== null) {
        const responseResult = await readAndLogResponse({
          requestSettings,
          responseText,
          requestId: "GENERATED_ID_PENDING", //TODO //requestResult.data.request.id,
          //dbClient,
          requestBody,
          responseStatus,
          startTime,
